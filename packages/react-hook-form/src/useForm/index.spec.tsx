...
import { useForm } from ".";
import { act, MockJSONServer, render, TestWrapper, waitFor } from "../../test";
...
describe("useForm hook", () => {
  ...
  it("should not resurrect removed useFieldArray rows from stale query data", async () => {
    ...
    // The field array lives in a child component, so the parent (which owns
    // useForm and its late-registration effect) does not re-render when the
    // row inputs mount. Its next render happens only after the user's
    // remove() — the point where stale query data must not be re-applied.
    const FieldArrayChild = ({ control }: { control: any }) => {
